...
import { REACT_APP_API_URL, productSize } from '../../config';
...
const AddProduct = ({ initialValues, ...props }: any) => {
	...
	useEffect(() => {
		setInsertProductData({
			...
			productSeason: getProductData?.getProduct ? getProductData?.getProduct?.productSeason : '',
	...
	const doDisabledCheck = () => {
		if (
			...
			insertProductData.productColor === 0 || // @ts-ignore
			insertProductData.productSeason === '' ||
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										...
										onChange={({ target: { value } }) =>
											//@ts-ignore
											setInsertProductData({ ...insertProductData, productColor: value })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
										</option>
										{productColor.map((color: any) => (
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertProductData.productSeason || 'select'}
										defaultValue={insertProductData.productSeason || 'select'}
										onChange={({ target: { value } }) =>
											//@ts-ignore
											setInsertProductData({ ...insertProductData, productSeason: value })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
										</option>
										{productSeason.map((season: any) => (
								...
								<Stack className="price-year-after-price">
									...
									<select
										...
										onChange={({ target: { value } }) =>
											setInsertProductData({ ...insertProductData, productSize: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
										...
										{productSize.map((size: number) => {
											if (size !== 0) {
												return <option value={parseInt(`${size}`)}>{size}</option>;
											}
										})}
									</select>
									<div className={'divider'}></div>
